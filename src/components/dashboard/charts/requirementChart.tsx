...
const jobColors: { gradient: string; solid: string; light: string }[] = [
  { gradient: 'from-rose-400 to-fuchsia-500', solid: '#F43F5E', light: '#FFE4E6' }, // Wedding Party
  { gradient: 'from-blue-400 to-cyan-400', solid: '#38BDF8', light: '#E0F2FE' },    // Baby Shower
  { gradient: 'from-orange-400 to-yellow-400', solid: '#F59E42', light: '#FEF3C7' },// Birthday
  { gradient: 'from-neutral-500 to-neutral-700', solid: '#6B7280', light: '#F3F4F6' }, // Corporate Event
  { gradient: 'from-green-400 to-lime-400', solid: '#22C55E', light: '#DCFCE7' },   // Graduation
  { gradient: 'from-purple-400 to-violet-400', solid: '#A78BFA', light: '#F3E8FF' },// Holiday
  { gradient: 'from-pink-400 to-red-400', solid: '#EC4899', light: '#FCE7F3' },     // Engagement
  { gradient: 'from-teal-400 to-emerald-400', solid: '#2DD4BF', light: '#CCFBF1' }, // Retirement
  { gradient: 'from-indigo-400 to-blue-500', solid: '#6366F1', light: '#E0E7FF' },  // Charity
  { gradient: 'from-yellow-400 to-amber-400', solid: '#FACC15', light: '#FEF9C3' }, // Family Reunion
];

const getJobTitle = (index: number) => JOB_TITLES[index] || `Special Event ${index + 1}`;
    ...
    <Card className={`flex flex-col w-full p-2 ${className}`}>
      ...
      <div className="space-y-2">
        {sortedData.map((item, index) => {
          ...
          const scheduledWidthPercent = item.inquiries > 0 ? (item.conversions / item.inquiries) * 100 : 0;
              ...
              <div className="flex items-center justify-between mb-1.5">
                <div className="flex flex-col">
                  ...
                  <span className="text-[8px] text-gray-500">
                    {item.inquiries} persons needed · {item.conversions} scheduled
                    {pending > 0 && (
